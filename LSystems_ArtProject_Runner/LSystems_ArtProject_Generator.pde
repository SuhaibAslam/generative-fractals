...
void generateImgs(int n_imgs, int n_rules) {
  for (int i = 0; i < n_imgs; i++) {
    int[] weight_vector = {0, 0, 0, 0, 0};
    IntList rule_indices = generateRuleIndices(n_rules);
    for (int j=0; j<rule_indices.size(); j++) {
      // Building a weight vector to trigger event
      int curr_rule_idx = rule_indices.get(j);
      weight_vector[curr_rule_idx] = (int) random(0, 101);
    }
    eventTrigger(weight_vector);
    delay(300);
    //save(str(i));
  }
}

IntList generateRuleIndices(int n_rules_) {
  IntList il = IntList.fromRange(0, 5);
  il.shuffle(this);
  il.resize(n_rules_);
  println(il);
  return il;
